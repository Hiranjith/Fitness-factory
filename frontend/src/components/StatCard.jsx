...
const StatCard = ({ title, value, subtitle, icon: Icon, colorClass, bgClass, onClick }) => {
  return (
    <div 
      onClick={onClick}
      className={`bg-surface rounded-2xl p-4 md:p-6 border border-border flex flex-col md:flex-row items-start md:items-center gap-3 md:gap-4 ${onClick ? 'cursor-pointer hover:bg-surface/80 transition-colors' : ''}`}
    >
      ...
      <div>
        ...
        {subtitle && <p className="text-text-secondary text-[10px] md:text-xs mt-0.5">{subtitle}</p>}
      </div>
    </div>
  );
};
